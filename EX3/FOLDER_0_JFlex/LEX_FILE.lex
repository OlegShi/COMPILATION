...
MINUS_SIGN		= -
LETTER_CHAR     = [A-Za-z0-9]
NUMBER          = [1-9][0-9]*
INTEGER			= 0 |{NUMBER} |{MINUS_SIGN}{NUMBER}
ID				= [A-Za-z]{LETTER_CHAR}*
String			= [\"]{LETTER_CHAR}*[\"]
...
{LineTerminator}	{ /* just skip what was found, do nothing */ }
 ...
 /* FROM EX1: {INTEGER}			{ 
 						...
 						if(num >= -32768 && num <= 32767)
                             //return symbol(TokenNames.INT, "INT("+num+")");
 ...
 {INTEGER}			{
                         try{
                             if (Integer.parseInt(yytext()) > 32767 || Integer.parseInt(yytext()) < -32768){                 
                                 //handling Ints larger than 2^15 -1 or smaller than -2^15
                                 return symbol(TokenNames.error);}
                             else{
                                 return symbol(TokenNames.INT, new Integer(yytext()));
                             }
                         } 
 				        /*In case of overflow, catch the error and return an ERROR token*/
 				        catch (Exception e){
 					       return symbol(TokenNames.error);}
 				    }
