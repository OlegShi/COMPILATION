...
public class AST_VAR_DEC extends AST_Node
{
	...
	/************************************************/
	public int localVariablesCounterGenerator = 0;
...
public void PrintMe()
{
 ...
 public TYPE SemantMe(TYPE_LIST father_data_members, boolean isGlobal, int GlobalVarsCount) throws SemanticException {
	 
	 TYPE t1 = param.SemantMe(father_data_members);
	
	 
   TYPE_VAR tv = new TYPE_VAR(param.name,t1);
    if(exp !=null && exp instanceof AST_EXP_INT){
      tv.initValue.intValue = ((AST_EXP_INT)exp).value;
    }
    
    if(exp !=null && exp instanceof AST_EXP_STRING){
      tv.initValue.isString = true;
      tv.initValue.stringValue = ((AST_EXP_STRING)exp).value;
    }
  ...
  SYMBOL_TABLE.getInstance().enter(param.name,t1);
    ...
    if (isGlobal) {
        SYMBOL_TABLE.getInstance().getEntry(param.name).setVariableInfo(4, globalVarsCount);
        globalVarsCount++;
    }
    else {
        //Seting stack offsets
        //if((t1 instanceof TYPE_INT) || (t1 instanceof TYPE_ARRAY)){
        localVariablesCounter = ++localVariablesCounterGenerator;
        SYMBOL_TABLE.getInstance().getEntry(param.name).localVariablesCounter = localVariablesCounter;
        //}
     }
     
   this.classification = SYMBOL_TABLE.getInstance().getEntry(param.name).getVariableClassification();
  ...
  public TEMP IRme()
	{
        ...
        if (exp != null)
        {
            ...
            init_value = TEMP_FACTORY.getInstance().getFreshTEMP();
            IR.getInstance().Add_IRcommand(new IRcommandConstInt(init_value,0));
        }
      
      //Global
        if (classification == 4){
            IR.getInstance().Add_IRcommand(new  IRcommand_store_global_var(
                init_value	,SYMBOL_TABLE.getInstance().getEntry(param.name).getOrder()));
        }
        // Local
            ...
            IR.getInstance().Add_IRcommand(new  IRcommand_store_local_var(
                init_value	,localVariablesCounter));
